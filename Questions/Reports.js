function formatQuestions(questionsObjArr) {
  return questionsObjArr.map((questionObj, i) => {
    ...
    const subItems = alternatives
      .reduce((tot, alt) => [...tot, { statement: removeHTMLTags(questionObj[alt]), correct: questionObj['Resp.'] === alt }], [])
    ...
    return {
      ...
      subItems,
      answer: `RESPOSTA: ${alternatives[subItems.reduce((tot, item, i) => item.correct ? i : tot, 0)]}`,
    };
  });
}
...
function appendQuestions(questionsObjList, documentBody, nestingLevel = 0) {
  ...
  questionsObjList.forEach((question, i) => {
    ...
    if (question.subItems && question.subItems.length > 0) appendQuestions(question.subItems, documentBody, nestingLevel + 1);
    if (question.answer) documentBody.appendParagraph(question.answer).setAttributes({
      [DocumentApp.Attribute.HORIZONTAL_ALIGNMENT]: DocumentApp.HorizontalAlignment.CENTER
    });
...
//  const filteredReferenceObjArr = clearEmptyFields(parseSpreadsheetAsObject([20, 1, 3]), 'IDs');
  ...
  const errors = appendQuestions(formattedQuestions, documentBody);
  document.saveAndClose();
  
  console.log(documentUrl);
  SpreadsheetApp.getUi().alert(`${documentUrl}\n${errors.length > 0 ? 'Erros:\n' + errors.map(e => e.message + '\n') : ''}`).OK;
}
